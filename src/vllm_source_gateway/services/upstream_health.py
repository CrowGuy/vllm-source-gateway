...
class UpstreamHealthMonitor:
    ...
    async def refresh_all(self) -> None:
        ...
        for upstream in self._routing_registry.health_snapshots():
            ...
            healthy = await self._probe_upstream(upstream_target.base_url)
    ...
    async def _probe_upstream(self, base_url: str) -> bool:
        assert self._client is not None
        probe_url = f"{base_url}{self._config.health.probe_path}"

        try:
            response = await self._client.get(probe_url, follow_redirects=True)
        except httpx.HTTPError as exc:
            logger.warning("upstream health probe failed", extra={"base_url": base_url, "error": str(exc)})
            return False
        ...
        if not healthy:
            logger.warning(
                ...
                extra={"base_url": base_url, "status_code": response.status_code},
            )
        return healthy
